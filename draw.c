 ...
 *
 * This example is written to run on Qualcomm Snapdragon 600 series board IFC6410
 * Current IFC6410 boards uses binary blobs for intializing hardware peripherals like DSP,
 * Internal cores, GPU Adreno etc.
 * Android BSP Rel v 1.5, which you need to download from Inforce Techweb. The ZIP file 
 * Inforce-IFC6410_AndroidBSP_880160_Rel_Beta_V1.5.zip includes all the proprietary firmware
 * blobs required for IFC6410 to intialize GPU at runtime which should get detected as /dev/dri/card0
 * by udevfs at boottime.
 *
 ...
 * Then follow the next instructions on how to extract firmware image included in the release
 * 
 * $ mkdir image
 ...
 * $ sudo cp -rf image/firmware /<path-to-ifc6410-rootfs>/lib/
 * $ rmdir image
 *
 *  
 * But it is very generic and run on any embedded device whose kernel support DRM/KMS
 * Also run you can test on your host machine
 ...
 * Our helper drmmodeset_setup_dev() returns -ENOENT if the connector is currently
 ...
 * drive one connector per CRTC! So we simply iterate through the "drmmodeset_list"
 ...
 * /dev/dri/card0. Then we open the device via drmmodeset_open(). drmmodeset_prepare()
 * prepares all connectors and we can loop over "drmmodeset_list" and call
 ...
 * drmmodeset_draw() that draws some colors into the framebuffer for 5 seconds and
 ...
 * This is used in drmmodeset_cleanup() to restore the CRTC to the same mode as was
 ...
 * interesting stuff is iterating over "drmmodeset_list" and then through all lines
 ...
 * drmmodeset_prepare(). It resets the CRTCs to their saved states and deallocates
 * all memory.
